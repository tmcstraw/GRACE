...
from config import SHAPE_DIR

SHP_DIR = SHAPE_DIR

from django.http import JsonResponse, HttpResponse, Http404

def process_shapefile(shapefile,url,uname,pwd,region_name,GRACE_NETCDF_DIR,GLOBAL_NETCDF_DIR,display_name,geoserver_id,storage_workspace):
    ...
    try:
        ...
        upload_tiff(geotiff_output_dir,region_name,url,storage_workspace,uname,pwd)
...
def process_shapefile_highres(shapefile,url,uname,pwd,region_name,GRACE_NETCDF_DIR,GLOBAL_NETCDF_DIR,display_name,geoserver_id,storage_workspace):
    ...
    try:
        ...
        for JS_dom_tot in range(IS_dom_tot):
            JS_grc_lat = IV_dom_lat[JS_dom_tot]
            ZS_grc_lat = ZV_grc_lat[JS_grc_lat]
            ZV_dom_sqm[JS_dom_tot] = 6371000 * math.radians(0.25) \
                                     * 6371000 * math.radians(0.25) \
                                     * math.cos(math.radians(ZS_grc_lat))
        ...
        if 'lwe_thickness' in f.variables:
            ...
            if 'long_name' in var.ncattrs(): lwe_thickness.long_name = var.long_name
            if 'units' in var.ncattrs(): lwe_thickness.units = var.units
            if 'coordinates' in var.ncattrs(): lwe_thickness.coordinates = var.coordinates
            if 'grid_mapping' in var.ncattrs(): lwe_thickness.grid_mapping = var.grid_mapping
            if 'cell_methods' in var.ncattrs(): lwe_thickness.cell_methods = var.cell_methods
...
def process_shapefile_highres_other_storage(shapefile,url,uname,pwd,region_name,GRACE_NETCDF_DIR,GLOBAL_NETCDF_DIR,display_name,geoserver_id,storage_workspace):
...
def process_shapefile_highres_end(shapefile,url,uname,pwd,region_name,GRACE_NETCDF_DIR,GLOBAL_NETCDF_DIR,display_name,geoserver_id,storage_workspace):
