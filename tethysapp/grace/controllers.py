...
def home(request):
    ...
    # upload_global_tiff("/Users/travismcstraw/tethysapps/GRACE/tethysapp/grace/workspaces/app_workspace/gracedata/tot_/global/geotiff_global/","http://tethys.byu.edu:8181/geoserver/rest/","tot_global",'admin','geoserver')
...
def map(request):
    ...
    select_storage_type = SelectInput(display_text='Select Storage Component',
                                    ...
                                    multiple=False,
                                    options= [('Total Water Storage (GRACE)', "http://127.0.0.1:8000/apps/grace/map/?region-select="+region_id), ('Surface Water Storage (GLDAS)', "http://127.0.0.1:8000/apps/grace/map/sw/?region-select="+region_id), ('Soil Moisture Storage (GLDAS)', "http://127.0.0.1:8000/apps/grace/map/soil/?region-select="+region_id), ('Groundwater Storage (Calculated)', "http://127.0.0.1:8000/apps/grace/map/gw/?region-select="+region_id)],
                                    initial=['']
                                    )
    ...
    context = {"region_id":region_id,"display_name":display_name,"wms_url":wms_url,"select_storage_type":select_storage_type,"select_layer":select_layer,"layers_length":layers_length,"grace_plot":grace_plot
...
def map_sw(request):
    ...
    select_storage_type = SelectInput(display_text='Select Storage Component',
                                    ...
                                    multiple=False,
                                    options= [('Total Water Storage (GRACE)', "http://127.0.0.1:8000/apps/grace/map/?region-select="+region_id), ('Surface Water Storage (GLDAS)', "http://127.0.0.1:8000/apps/grace/map/sw/?region-select="+region_id), ('Soil Moisture Storage (GLDAS)', "http://127.0.0.1:8000/apps/grace/map/soil/?region-select="+region_id), ('Groundwater Storage (Calculated)', "http://127.0.0.1:8000/apps/grace/map/gw/?region-select="+region_id)],
                                    initial=['Surface Water Storage (GLDAS)']
                                    )
    ...
    return render(request, 'grace/map_sw.html', context)
...
def map_soil(request):
    ...
    select_storage_type = SelectInput(display_text='Select Storage Component',
                                    ...
                                    multiple=False,
                                    options= [('Total Water Storage (GRACE)', "http://127.0.0.1:8000/apps/grace/map/?region-select="+region_id), ('Surface Water Storage (GLDAS)', "http://127.0.0.1:8000/apps/grace/map/sw/?region-select="+region_id), ('Soil Moisture Storage (GLDAS)', "http://127.0.0.1:8000/apps/grace/map/soil/?region-select="+region_id), ('Groundwater Storage (Calculated)', "http://127.0.0.1:8000/apps/grace/map/gw/?region-select="+region_id)],
                                    initial=['Soil Moisture Storage (GLDAS)']
                                    )
    ...
    grace_plot = TimeSeries(
        engine='highcharts',
        title=display_name+ ' GLDAS Data',
        y_axis_title='Total Soil Moisture Storage Anomaly',
        y_axis_units='cm',
        series=[
            {
                'name': 'Height of Liquid Water',
    ...
    return render(request, 'grace/map_soil.html', context)
...
def map_gw(request):
    ...
    select_storage_type = SelectInput(display_text='Select Storage Component',
                                    ...
                                    multiple=False,
                                    options= [('Total Water Storage (GRACE)', "http://127.0.0.1:8000/apps/grace/map/?region-select="+region_id), ('Surface Water Storage (GLDAS)', "http://127.0.0.1:8000/apps/grace/map/sw/?region-select="+region_id), ('Soil Moisture Storage (GLDAS)', "http://127.0.0.1:8000/apps/grace/map/soil/?region-select="+region_id), ('Groundwater Storage (Calculated)', "http://127.0.0.1:8000/apps/grace/map/gw/?region-select="+region_id)],
                                    initial=['Groundwater Storage (GLDAS)']
                                    )
    ...
    grace_plot = TimeSeries(
        engine='highcharts',
        title=display_name+ ' GRACE Groundwater Data',
        y_axis_title='Total Groundwater Storage Anomaly',
        y_axis_units='cm',
        series=[
            {
                'name': 'Height of Liquid Water',
    ...
    return render(request, 'grace/map_gw.html', context)
...
def global_map(request):
    ...
    select_storage_type = SelectInput(display_text='Select Storage Component',
                                    name = 'select_storage_type',
                                    multiple=False,
                                    options= [('Total Water Storage (GRACE)', "http://127.0.0.1:8000/apps/grace/global-map/"), ('Surface Water Storage (GLDAS)', "http://127.0.0.1:8000/apps/grace/global-map/sw/"), ('Soil Moisture Storage (GLDAS)', "http://127.0.0.1:8000/apps/grace/global-map/soil/"), ('Groundwater Storage (GRACE)', "http://127.0.0.1:8000/apps/grace/global-map/gw")],
                                    initial=['']
                                    )
...
def global_map_sw(request):

    color_bar = get_color_bar()
    color_bar = json.dumps(color_bar)

    grace_layer_options = get_global_dates(SW_GLOBAL_NETCDF_DIR)
    ...
    return render(request, 'grace/global_map_sw.html', context)

def global_map_soil(request):

    color_bar = get_color_bar()
    color_bar = json.dumps(color_bar)

    grace_layer_options = get_global_dates(SOIL_GLOBAL_NETCDF_DIR)
    ...
    return render(request, 'grace/global_map_soil.html', context)

def global_map_gw(request):

    color_bar = get_color_bar()
    color_bar = json.dumps(color_bar)

    grace_layer_options = get_global_dates(GW_GLOBAL_NETCDF_DIR)
    ...
    select_storage_type = SelectInput(display_text='Select Storage Component',
                                    ...
                                    multiple=False,
                                    options= [('Total Water Storage (GRACE)', "http://127.0.0.1:8000/apps/grace/global-map/"), ('Surface Water Storage (GLDAS)', "http://127.0.0.1:8000/apps/grace/global-map/sw/"), ('Soil Moisture Storage (GLDAS)', "http://127.0.0.1:8000/apps/grace/global-map/soil/"), ('Groundwater Storage (GRACE)', "http://127.0.0.1:8000/apps/grace/global-map/gw")],
                                    initial=['Groundwater Storage (GRACE)']
                                    )
    ...
    return render(request, 'grace/global_map_gw.html', context)
