...
def create_global_tiff(file_name,output_dir,var_name):
    ...
    for timestep, v in enumerate(time):
        ...
        DataSet = None
    print('Global Geotiff Creation Complete')


def create_global_tiff_highres(file_name,output_dir,var_name):
    print('Creating Global Geotiffs')
    output_dir = os.path.join(output_dir, '')
    ...
    for timestep, v in enumerate(time):
        ...
        driver = gdal.GetDriverByName('GTiff')
        DataSet = driver.Create(output_dir+ts_file_name+'.tif',xsize,ysize,1, gdal.GDT_Float32)
        DataSet.SetGeoTransform([-180.0, 0.25, 0.0, 90, 0.0, -0.25])
        srs=osr.SpatialReference()
        srs.ImportFromEPSG(4326)
        DataSet.SetProjection(srs.ExportToWkt())
...
def upload_global_tiff(dir,geoserver_rest_url,workspace,uname,pwd):
    print('Uploading Geotiffs to Geoserver')
    headers = {
...
def get_global_plot(pt_coords,GLOBAL_DIR):
    graph_json = {}

    ts_plot = []

    nc_file = get_global_nc(GLOBAL_DIR)
...
def get_global_poly(bounds,GLOBAL_DIR):
    graph_json = {}
...
def get_global_plot_api(pt_coords,start_date,end_date,GLOBAL_DIR):
...
def get_global_dates(GLOBAL_DIR):
    grace_layer_options = []
    grace_nc = None
    for file in os.listdir(GLOBAL_DIR):
        if file.startswith('GRC') and file.endswith('.nc'):
